...
function deriveDefaults(booking?: Booking): Partial<BookingFormValues> {
  if (!booking) {
    return {
      hotel_name: "",
      currency: "GBP",
  ...
  return {
    ...
    room_specific: booking.room_specific,
    num_guests: booking.num_guests,
    current_price: booking.current_price,
    currency: booking.currency,
    cancellation_date: booking.cancellation_date.slice(0, 10),
    cancellation_url: booking.cancellation_url,
    original_booking_source: booking.original_booking_source,
    original_confirmation: booking.original_confirmation,
    threshold_percent: booking.threshold_percent,
    threshold_absolute: booking.threshold_absolute,
    non_refundable_window_days: booking.non_refundable_window_days,
  };
}
...
export function BookingForm({ mode, initialData, bookingId }: BookingFormProps) {
  ...
  const title = isEdit ? "Edit Booking" : "Add Booking";
  const submitLabel = isEdit ? "Save Changes" : "Add Booking";
  const cancelHref = isEdit ? `/bookings/${bookingId}` : "/";
  ...
  async function onSubmit(data: BookingFormValues) {
    ...
    router.push(isEdit ? `/bookings/${bookingId}` : "/");
  }
